...
import { getTaskById, getUserById, acknowledgeTask } from '@/lib/api';
  ...
  useEffect(() => {
    ...
    async function loadTask() {
      ...
      try {
        ...
        if (taskData?.assignedMemberId) {
          const canFetchAssigned =
            user?.role === 'admin' || user?.role === 'team_leader' || String(taskData.assignedMemberId) === String(user?.id);
          if (canFetchAssigned) {
            try {
              ...
              // ignore unauthorized access for assigned user details
            }
          }
        }

        if (taskData?.createdBy) {
          const canFetchCreator =
            user?.role === 'admin' || user?.role === 'team_leader' || String(taskData.createdBy) === String(user?.id);
          if (canFetchCreator) {
            try {
              ...
              // ignore unauthorized access for creator details
            }
          }
        }
      } catch (err: any) {
